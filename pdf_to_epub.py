...
class PdfToEpubConverter:
    ...
    def analyze_page(self, page_num):
        ...
        # Add remaining images as raw items (to be inserted in place if no reference found)
        # We wrap them to look like sortable items
        ...
        for item in raw_items:
            ...
            # If item usually spans > 70% of page width, it's full width (like a header)
            if w > width * 0.7:
        ...
        # B. Sort each list by Y
        left_col.sort(key=lambda x: x['bbox'].y0)
        right_col.sort(key=lambda x: x['bbox'].y0)
        full_width.sort(key=lambda x: x['bbox'].y0)

        # C. Merge: Heuristically, full_width usually top or bottom. 
        # But for papers, usually Header -> Left -> Right -> Footer.
        # We will iterate vertically. 
        # Simpler approach: Just output Left then Right. Headers usually appear in Left queue if x0 is small.
        # Let's trust the clean split for now.
        
        sorted_items = left_col + right_col
